            ...
            <div className={`p-3 rounded-lg border shadow-lg ${isDarkMode ? 'bg-card border-border text-card-foreground' : 'bg-white border-slate-200 text-slate-800'}`}>
                ...
                <div className="text-sm space-y-1">
                    <p className="flex items-center gap-2">
                        ...
                        Steps: <span className="font-mono font-medium">{formatGraphNumber(data.steps)}</span>
                    </p>
                    <p className="flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-emerald-500" />
                        Calories: <span className="font-mono font-medium">{formatGraphNumber(data.calories)} kcal</span>
                    </p>
                </div>
            </div>
        );
            ...
            <ResponsiveContainer width="100%" height={350}>
                ...
                >
                    ...
                    <XAxis
                        ...
                        fontSize={12}
                        minTickGap={32}
                        tickFormatter={(value) => formatGraphNumber(value)}
                        allowDecimals={false}
                    >
                    ...
                    {isChartInteractionEnabled && <Tooltip content={<CustomTooltip isDarkMode={isDarkMode} />} trigger="click" />}
                    <Scatter
                        name="Daily Log"
                        data={data}
                        fill="#8884d8"
